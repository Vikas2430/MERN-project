  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setBook(response.data.data || []); // Safely handle undefined or null
        setLoading(false);
      })
      .catch((error) => {
        console.error('Error fetching data:', error);
        setBook([]); // Set an empty array in case of error
        setLoading(false);
      });
  }, []);
            ...
            {book && book.length > 0 ? (
              book.map((book, index) => (
                <tr key={book._id} className="h-8">
                  ...
                  <td className="border border-slate-600 rounded-md text-center hidden md:table-cell">{book.author}</td>
                  <td className="border border-slate-600 rounded-md text-center hidden md:table-cell">{book.publishYear}</td>
